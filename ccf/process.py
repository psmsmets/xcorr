...
class CC:
    ...
    def cc(
        ...
        pad: bool = True, unbiased: bool = True, dtype: np.dtype = np.float32
    ):
        """
        Returns the cross-correlation estimate for vectors `x` and `y`.
        Crosscorrelation is performed in the frequency domain.
        """
        n = len(x)
    ...
    def lag(
        ...
        """
        Returns an array with lag times given the number of samples `n`
        and time step `delta`.
        """
    ...
    def weight(
        ...
        """
        Returns an array with scale factors to obtain the unbaised
        crosscorrection estimate.
        """
        nn = n*2-1 if pad else n
        n = np.int((nn+1)/2)
    ...
    def extract_shift(
        cc: np.ndarray, delta: float = None
    ):
        """
    ...
    def extract_shift_and_max(
        Rxy, delta: float = None
    ):
        """
        Returns the sample (or time) shift at the maximum of
        crosscorrelation estimate `Rxy` and the maximum.
        """
    ...
    def compute_shift_and_max(
        ...
        """
        Returns the sample (or time) shift at the maximum of
        crosscorrelation estimate and the maximum.
        """
...
class Preprocess:
        ...
        for operation_params in operations:
            ...
            try:
                st = Preprocess.apply_stream_operation(
                    stream=st,
                    operation=operation,
                    parameters=Preprocess.inject_parameters(
    ...
    def add_operations_to_DataArray(
        darray: xr.DataArray, preprocess: dict,
    ...
    def running_rms(signal, **kwargs):
        """
        Returns a new `obspy.Stream` or `obpsy.Trace` with the running
...
class Postprocess:
    ...
    _d360_ = [0, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30]
    _d365_ = [0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    _d366_ = [0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    ...
    dpm = {
        ...
        'standard': _d365_,
        'gregorian': _d365_,
        'proleptic_gregorian': _d365_,
        'all_leap': _d366_,
        '366_day': _d366_,
        '360_day': _d360_,
    }

    def leap_year(year, calendar: str = 'standard'):
        """
    ...
    def get_dpm(time, calendar: str = 'standard'):
        """
    ...
    def stack_year_month(
        ...
        month_length = xr.DataArray(
            Postprocess.get_dpm(dataset.time.to_index()),
            coords=[dataset.time],
            name='month_length'
        )
        weights = (
    ...
    def stack_year_dayofyear(
        ...
        return (
            dataset[Postprocess.list_variables(dataset, **kwargs)]
